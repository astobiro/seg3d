...
class Unet3Dmodel:
    ...
    def evaluate_model_per_patient(self,n_steps=None,shuffle=False):
        ...
        for i in range(len(average_metrics["IoU"])):
            average_dict.loc[self.config.segmentation_name_map[i], "IoU"] = average_metrics["IoU"][i]
            average_dict.loc[self.config.segmentation_name_map[i], "DICE"] = average_metrics["DICE"][i]
